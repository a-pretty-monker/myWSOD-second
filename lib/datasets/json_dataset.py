...
class JsonDataset(object):
    ...
    def get_roidb(
        ...
        catch_filepath_me = os.path.join(self.cache_path, self.name + '_me_roidb.pkl')
        if os.path.exists(catch_filepath_me) and not cfg.DEBUG:
            self.debug_timer.tic()
            logger.info('Loading cached roidb from %s', catch_filepath_me)
            with open(catch_filepath_me, 'rb') as fp:
                roidb = pickle.load(fp)
            logger.debug(
                '_add_roidb_from_cache took {:.3f}s'.
        ...
        # catch_filepath_me = os.path.join(self.cache_path, self.name+'_me_roidb.pkl')
        # with open(catch_filepath_me, 'wb') as fp:
        #     pickle.dump(roidb, fp, pickle.HIGHEST_PROTOCOL)
        # logger.info('Cache my roidb to %s', catch_filepath_me)
        return roidb
    ...
    def _prep_roidb_entry(self, entry):
        ...
        # entry['edges'] = np.empty((0, 2), dtype=np.int32)
...
def _merge_proposal_boxes_into_roidb(roidb, box_list):
    ...
    for i, entry in enumerate(roidb):
        boxes = box_list[i]
        # edges = np.array(box_utils.calculate_edges2(boxes.astype(dtype=np.float32, copy=False)))
        entry['boxes'] = np.append(
            entry['boxes'],
            boxes.astype(entry['boxes'].dtype, copy=False),
            axis=0
        )
        # entry['edges'] = np.append(
        #     entry['edges'],
        #     edges.astype(entry['edges'].dtype, copy=False),
        #     axis=0
        # )
        # print(i)
