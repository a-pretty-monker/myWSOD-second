...
def calculate_edges(boxes):
    matrix = bbox_overlaps(boxes,boxes)
    mask = 1 - np.diag(np.ones(matrix.shape[0]))
    new = np.nonzero((matrix >= 0.8) * mask)
    # indices = np.vstack((new.row, new.col))  # 我们真正需要的coo形式
    new = torch.LongTensor(new)  # PyG框架需要的coo形式
    return new
