...
def get_minibatch(roidb, num_classes):
    ...
    # edges_blob = np.zeros((0,3),dtype=np.int32)
    ...
    for im_i in range(num_images):
        ...
        # Add to Edges blob
        # batch_ind = im_i * np.ones((edges.shape[0], 1))
        # edges_blob_this_image = np.hstack((batch_ind, edges))

        # if cfg.DEDUP_BOXES > 0:
        #     print(rois_blob_this_image.shape[0])
        ...
        # edges_blob = np.vstack((edges_blob, edges_blob_this_image))

        # Add to labels blob
        labels_blob = np.vstack((labels_blob, labels))

    blobs['rois'] = rois_blob
    # blobs['edges'] = edges_blob
